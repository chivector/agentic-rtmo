...
from .agentic_modules import ThinkCritiqueActLoop
...
class DCC(BaseModule):
    ...
    def __init__(
        ...
        agentic_cfg: Optional[dict] = None,
        ...
        self.agentic_cfg = copy.deepcopy(agentic_cfg) if agentic_cfg else {}
        self.num_agentic_iters = int(self.agentic_cfg.get('num_iters', 2))
        self.use_agentic_loop = bool(self.agentic_cfg.get('enabled', False))
        ...
        if self.use_agentic_loop:
            self.agentic_loop = ThinkCritiqueActLoop(
                num_keypoints=self.num_keypoints,
                feat_dim=self.feat_channels,
                critic_hidden_dim=self.agentic_cfg.get('critic_hidden_dim', 64),
                actor_hidden_dim=self.agentic_cfg.get('actor_hidden_dim', 128),
                residual_scale=self.agentic_cfg.get('residual_scale', 0.5))
    ...
    def _pose_feats_to_kpt_feats(self, pose_feats: Tensor) -> Tensor:
        kpt_feats = self.pose_to_kpts(pose_feats)
        kpt_feats = kpt_feats.reshape(*kpt_feats.shape[:-1], self.num_keypoints,
                                      self.feat_channels)
        if hasattr(self, 'gau'):
            kpt_feats = self.gau(
    ...
    def _kpt_feats_to_heatmaps(self, kpt_feats: Tensor, x_bins_enc: Tensor,
                               y_bins_enc: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    def _pose_feats_to_heatmaps(self, pose_feats, x_bins_enc, y_bins_enc):
        ...
        """
        kpt_feats = self._pose_feats_to_kpt_feats(pose_feats)
        return self._kpt_feats_to_heatmaps(kpt_feats, x_bins_enc, y_bins_enc)
    ...
    def forward_train(self, pose_feats, bbox_cs, grids):
        ...
        if self.use_agentic_loop and self.num_agentic_iters > 0:
            for _ in range(self.num_agentic_iters):
                x_hms_iter, y_hms_iter = self._kpt_feats_to_heatmaps(
                    kpt_feats, x_bins_enc, y_bins_enc)
                x_prob_iter, y_prob_iter = self._apply_softmax(
                    x_hms_iter, y_hms_iter)
                pose_iter = self._decode_xy_heatmaps(x_prob_iter, y_prob_iter,
                                                     bbox_cs)
                joint_scores = 0.5 * (x_prob_iter.max(dim=-1).values +
                                      y_prob_iter.max(dim=-1).values)
                kpt_feats, _, _ = self.agentic_loop(kpt_feats, pose_iter,
                                                    joint_scores)

        x_hms, y_hms = self._kpt_feats_to_heatmaps(kpt_feats, x_bins_enc,
                                                   y_bins_enc)
    ...
    def switch_to_deploy(self, test_cfg: Optional[Dict] = None):
        ...
        if self.use_agentic_loop:
            # Keep original forward path to preserve iterative refinement.
            self.deploy = True
            return
